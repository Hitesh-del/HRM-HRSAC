...
import {
  ...
  RefreshCw, FileDown, Eye, Edit, Trash2, FileText, BarChart3,
  GraduationCap, Calendar, Building2, Phone, Mail
} from 'lucide-react';
...
interface ManagerRow { id: string; full_name?: string; department_id?: string }
interface InternRow {
  ...
  profile?: { id: string; full_name?: string; email?: string; employee_id?: string; is_active?: boolean; department?: { name: string } | null } | null;
...
export default function DirectorInternship() {
  ...
  const fetchAttendance = useCallback(async () => {
    setAttLoading(true);
    // Join attendance → profiles (filter role=intern) → departments
    const { data, error } = await supabase
      .from('attendance')
      .select(`id, date, check_in_time, check_out_time, working_hours, status,
        employee:profiles!attendance_employee_id_fkey(employee_id, full_name, role,
          department:departments!profiles_department_id_fkey(name))`)
      .eq('date', attDate)
      .order('check_in_time', { ascending: true });
    if (error) console.error('fetchAttendance error:', error.message);
    // filter to intern role client-side
    setAttRecords(((data || []) as unknown as InternAttRow[]).filter(r => (r.employee as any)?.role === 'intern'));
    setAttLoading(false);
  }, [attDate]);

  const fetchAll = useCallback(async () => {
    setLoading(true);
    const [internsRes, deptsRes, mgrsRes] = await Promise.all([
      supabase.from('intern_details').select(`
        ...
        profile:profiles!intern_details_profile_id_fkey(id,full_name,email,employee_id,is_active,department:departments!profiles_department_id_fkey(name)),
        reporting_manager:profiles!intern_details_reporting_manager_id_fkey(full_name)
      `).order('created_at', { ascending: false }),
      supabase.from('departments').select('id,name').order('name'),
      supabase.from('profiles').select('id,full_name,department_id').in('role',['director','management']).eq('is_active',true).order('full_name'),
    ]);
    setInterns((internsRes.data || []) as unknown as InternRow[]);
    setDepartments((deptsRes.data || []) as DeptRow[]);
  ...
  useEffect(() => {
    fetchAll();
    fetchAttendance();
    const ch = supabase.channel('director-interns-rt')
  ...
  useEffect(() => {
    ...
    const expired = interns.filter(i => {
      const s = computeStatus(i.start_date, i.end_date, i.status);
      return s === 'expired' && i.status !== 'expired';
    });
    expired.forEach(i => {
      supabase.from('intern_details').update({ status: 'expired' }).eq('id', i.id).then(() => {});
    });
  }, [interns]);

  const filtered = interns.filter(i => {
    const name = (i.profile as any)?.full_name?.toLowerCase() || '';
    const email = (i.profile as any)?.email?.toLowerCase() || '';
    const dept = (i.profile as any)?.department?.name?.toLowerCase() || '';
    const q = search.toLowerCase();
    const matchQ = !q || name.includes(q) || email.includes(q) || dept.includes(q) || (i.internship_role || '').toLowerCase().includes(q);
    const displayStatus = computeStatus(i.start_date, i.end_date, i.status);
    const matchStatus = statusFilter === 'all' || displayStatus === statusFilter;
    const matchDept = deptFilter === 'all' || dept === deptFilter.toLowerCase();
  ...
  const handleSubmit = async () => {
    if (!form.full_name.trim() || !form.email.trim() || !form.department_id || !form.start_date || !form.end_date) {
      toast.error('Full Name, Email, Department, Start Date, and End Date are required');
      return;
    }
    if (!editIntern && !form.password.trim()) { toast.error('Password is required for new intern accounts'); return; }
    ...
    try {
      if (editIntern) {
        ...
        }).eq('id', editIntern.profile_id);
        if (profErr) { toast.error(profErr.message); setSaving(false); return; }

        // Compute duration
        const durMonths = parseFloat(
          (((new Date(form.end_date).getTime() - new Date(form.start_date).getTime()) / (1000 * 60 * 60 * 24 * 30.44))).toFixed(1)
        );

        const { error: detErr } = await supabase.from('intern_details').update({
        ...
        if (detErr) { toast.error(detErr.message); setSaving(false); return; }
        toast.success('Intern updated successfully');
      } else {
        // Create intern via Edge Function (service_role — never hijacks current director session)
  ...
  const openEdit = (i: InternRow) => {
    setEditIntern(i);
    setForm({
      full_name: (i.profile as any)?.full_name || '',
      email: (i.profile as any)?.email || '',
      password: '',
      mobile_number: i.mobile_number || '',
      department_id: '', // will auto-fill
      internship_role: i.internship_role || '',
      college_name: i.college_name || '',
      start_date: i.start_date,
      end_date: i.end_date,
      reporting_manager_id: i.reporting_manager_id || '',
      notes: i.notes || '',
    });
    // Fetch dept id
    supabase.from('profiles').select('department_id').eq('id', i.profile_id).maybeSingle().then(({ data }) => {
      if (data?.department_id) setForm(p => ({ ...p, department_id: data.department_id }));
    });
    setShowAdd(true);
  };

  const handleDelete = async (id: string, profileId: string) => {
    if (!confirm('Delete this intern? This will remove their account permanently.')) return;
    setDeleting(id);
    await supabase.from('intern_details').delete().eq('id', id);
    await supabase.from('profiles').update({ is_active: false, role: 'employee' }).eq('id', profileId);
  ...
  const handleToggleAccount = async (intern: InternRow, disable: boolean) => {
    ...
    try {
      // Update account_disabled flag on intern_details
      ...
      // Also flip is_active on profile so login check works immediately
      ...
      // Fire-and-forget notification to intern
      if (disable) {
    ...
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex items-start justify-between flex-wrap gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage all intern accounts across departments</p>
        </div>
      ...
      <Tabs defaultValue="interns">
        ...
        <TabsContent value="interns" className="mt-4 space-y-3">
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground font-medium">No interns found</p>
              <p className="text-sm text-muted-foreground">Add your first intern using the button above</p>
            </div>
          ) : (
            <Card>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      {['Intern ID','Name','Department','Email','Role','Duration','Start','End','Reporting Manager','Status','Account Status','Actions'].map(h => (
                    ...
                    {filtered.map(i => {
                      ...
                      return (
                        <tr key={i.id} className="border-b border-border hover:bg-muted/20 transition-colors">
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap text-xs text-muted-foreground">{(i.profile as any)?.department?.name || '—'}</td>
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap text-xs">
                            <div>{new Date(i.end_date).toLocaleDateString()}</div>
                            {disp === 'active' && rem > 0 && <div className="text-orange-400 text-xs">{rem}d left</div>}
                          </td>
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap">
                            <div className="flex flex-wrap gap-1">
                              ...
                              <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-400 hover:text-red-400" disabled={deleting === i.id} onClick={() => handleDelete(i.id, i.profile_id)}><Trash2 className="w-3.5 h-3.5" /></Button>
                            </div>
                          </td>
                        </tr>
                      );
        ...
        <TabsContent value="reports" className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* By Dept */}
            <Card>
              <CardHeader><CardTitle className="text-sm">Interns by Department</CardTitle></CardHeader>
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                <table className="w-full text-sm min-w-max">
                  <thead><tr className="border-b border-border"><th className="text-left px-4 py-2 text-xs text-muted-foreground whitespace-nowrap">Department</th><th className="text-right px-4 py-2 text-xs text-muted-foreground whitespace-nowrap">Interns</th></tr></thead>
                  <tbody>
                    {departments.filter(d => interns.some(i => (i.profile as any)?.department?.name === d.name)).map(d => (
                      <tr key={d.id} className="border-b border-border">
                        <td className="px-4 py-2 text-foreground whitespace-nowrap">{d.name}</td>
                        <td className="px-4 py-2 text-right font-bold text-primary whitespace-nowrap">
                          {interns.filter(i => (i.profile as any)?.department?.name === d.name).length}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {/* By Status */}
            <Card>
              <CardHeader><CardTitle className="text-sm">Internship Completion Report</CardTitle></CardHeader>
        ...
        <TabsContent value="attendance" className="mt-4 space-y-4">
          <div className="flex flex-wrap gap-2 items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={fetchAttendance} disabled={attLoading} className="gap-1.5">
                <RefreshCw className={`w-3.5 h-3.5 ${attLoading ? 'animate-spin' : ''}`} /> Refresh
              </Button>
              <Button variant="outline" size="sm" onClick={() => setExportInternOpen(true)} className="gap-1.5">
                <FileDown className="w-3.5 h-3.5" /> Export
              </Button>
            </div>
          </div>
                  ...
                  {attLoading
                    ...
                    : (() => {
                        ...
                        if (!filtered.length) return (
                          <tr><td colSpan={8} className="px-4 py-8 text-center text-muted-foreground text-sm">No intern attendance records for this date.</td></tr>
                        );
                        const fmtTime = (t: string | null | undefined) => t ? new Date(t).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—';
                        const statusBadge: Record<string, string> = {
                          ...
                          half_day:'border-blue-500/30 text-blue-400 bg-blue-500/10',
                        };
                        return filtered.map(r => (
                          <tr key={r.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-xs text-muted-foreground">{r.date || attDate}</td>
                            <td className="px-4 py-3 whitespace-nowrap font-mono text-xs text-green-400">{fmtTime(r.check_in_time)}</td>
                            <td className="px-4 py-3 whitespace-nowrap font-mono text-xs text-blue-400">{fmtTime(r.check_out_time)}</td>
                            <td className="px-4 py-3 whitespace-nowrap text-center">{r.working_hours != null ? `${r.working_hours}h` : '—'}</td>
                            <td className="px-4 py-3 whitespace-nowrap">
                              <Badge variant="outline" className={`text-xs capitalize ${statusBadge[r.status] || ''}`}>
      ...
      {/* Intern Attendance Export Dialog */}
      <ReportExportDialog
        open={exportInternOpen}
        onClose={() => setExportInternOpen(false)}
      ...
      <ReportExportDialog
        ...
        columns={[
          ...
          { header: 'Department',   key: 'profile', format: v => (v as any)?.department?.name || '—' },
      ...
      <Dialog open={showAdd} onOpenChange={v => { if (!v) { setShowAdd(false); setEditIntern(null); } }}>
        <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-2xl max-h-[90dvh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editIntern ? 'Edit Intern' : 'Add New Intern'}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="email" value={form.email} onChange={f('email')} placeholder="john@example.com" disabled={!!editIntern} />
            </div>
            {!editIntern && (
              <div className="space-y-1.5">
                <Label>Login Password <span className="text-red-400">*</span></Label>
                <Input type="password" value={form.password} onChange={f('password')} placeholder="Minimum 6 characters" />
              </div>
            )}
            <div className="space-y-1.5">
              <Label>Mobile Number</Label>
              <Input value={form.mobile_number} onChange={f('mobile_number')} placeholder="+1 234 567 8900" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.internship_role} onChange={f('internship_role')} placeholder="e.g. Software Engineering Intern" />
            </div>
            <div className="space-y-1.5">
              <Label>College / Institute Name</Label>
              <Input value={form.college_name} onChange={f('college_name')} placeholder="e.g. MIT" />
            </div>
            <div className="space-y-1.5">
              ...
              <Select value={form.reporting_manager_id} onValueChange={v => setForm(p => ({ ...p, reporting_manager_id: v }))}>
                ...
                <SelectContent>
                  {managers
                    .filter(m => !form.department_id || m.department_id === form.department_id || m.id === myProfile?.id)
            ...
            return (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                  {[
                    ['Department', (viewIntern.profile as any)?.department?.name || '—'],
                    ...
                    ['Joined', new Date(viewIntern.created_at).toLocaleDateString()],
                  ].map(([k,v]) => (
                    <div key={k}><p className="text-xs text-muted-foreground">{k}</p><p className="font-medium text-foreground text-sm">{v}</p></div>
                  ))}
                </div>
                {viewIntern.notes && <div><p className="text-xs text-muted-foreground mb-1">Notes</p><p className="text-sm text-foreground bg-muted/30 rounded p-3 text-pretty">{viewIntern.notes}</p></div>}
