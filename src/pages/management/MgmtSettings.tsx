...
import { Lock, Bell, User, Eye, EyeOff, Camera, Trash2 } from 'lucide-react';
...
export default function MgmtSettings() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 min-w-0">
      <div><h1 className="text-xl font-bold text-foreground text-balance">Settings</h1><p className="text-sm text-muted-foreground">Account settings</p></div>
      <Tabs defaultValue="profile">
        ...
        <TabsContent value="profile" className="mt-4 space-y-4">
          ...
          <Card className="max-w-lg">
            ...
            <CardContent className="flex items-center flex-wrap gap-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground capitalize">{profile?.role} · {profile?.employee_id}</p>
          ...
          {/* Profile info card */}
          <Card className="max-w-lg">
            <CardHeader><CardTitle className="text-sm flex items-center flex-wrap gap-2"><User className="w-4 h-4"/>Profile Info</CardTitle></CardHeader>
        ...
        <TabsContent value="security" className="mt-4 space-y-4">
          <Card className="max-w-lg">
            ...
            <CardContent className="space-y-4">
              <div><label className="text-sm font-normal text-foreground block mb-1.5">New Password</label><div className="relative"><Input type={showNew?'text':'password'} value={newPassword} onChange={e=>setNewPassword(e.target.value)} className="pr-9"/><button type="button" onClick={()=>setShowNew(!showNew)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">{showNew?<EyeOff className="w-4 h-4"/>:<Eye className="w-4 h-4"/>}</button></div></div>
        ...
        <TabsContent value="notifications" className="mt-4">
          <Card className="max-w-lg">
            ...
            <CardContent className="space-y-4">
              {['Leave Request Alerts','Task Assignments','Project Updates','Attendance Alerts'].map(l=>(
